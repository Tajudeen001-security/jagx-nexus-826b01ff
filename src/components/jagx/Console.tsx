...
function readFile(file: File): Promise<Att> {
  return new Promise((resolve, reject) => {
    ...
    r.onload = () => resolve({ name: file.name, mime: file.type || "text/plain", dataUrl: String(r.result) });
...
export function Console({
  grade, onGrade, messages, setMessages,
}: {
  grade: GradeId;
  ...
  async function pick(list: FileList | null) {
    ...
    for (const f of Array.from(list).slice(0, 6)) {
      if (f.size > MAX_BYTES) {
        setMessages((m) => [...m, { role: "assistant", content: `${f.name} is larger than 8 MB.`, error: true }]);
        continue;
      }
  ...
  const needsAgent = (message: string, files: Att[]) => files.length > 0 || /\b(build|create|generate|write|code|debug|fix|refactor|test|run|verify|inspect|scan|browse|research|investigate|compare|analy[sz]e|package|zip|epub|docx|pdf|rtf|txt|website|project|repo|github|supabase|deploy)\b/i.test(message);\n\n  async function submit(text: string) {
    ...
    const files = atts;
    setMessages((m) => [...m, { role: "user", content: message, files: files.map((f) => f.name) }]);
    setInput("");
    setAtts([]);
    setBusy(true);
    try {
      ...
      if (files.length) {
        const { text: digest } = await analyze({
          data: {
            prompt: message || "Read these files and give a precise, structured summary plus anything useful for building with them.",
            attachments: files,
          },
        });
          ...
          {messages.map((m, i) =>
            m.role === "user" ? (
                ...
                <div className="max-w-[85%] rounded-2xl rounded-br-sm bg-surface-2 px-4 py-2.5 text-sm">
                  {m.files && m.files.length > 0 && (
                    <div className="mb-1 flex flex-wrap gap-1.5 font-mono text-[10px] text-muted-foreground">
                      {m.files.map((f) => (
                        <span key={f} className="rounded border border-border px-1.5 py-0.5">{f}</span>
                      ))}
                    </div>
                  )}
                  {m.content}
              ...
              <div key={i} className="space-y-3">
                ...
                {m.image && <img src={m.image} alt={m.content} className="max-w-full rounded-xl border border-border" />}
        ...
        <div className="mx-auto w-full max-w-3xl space-y-2">
          ...
          {atts.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {atts.map((a, i) => (
                <span key={`${a.name}-${i}`} className="flex items-center gap-1.5 rounded-full border border-border px-2.5 py-1 font-mono text-[10px] text-muted-foreground">
          ...
          <input ref={fileRef} type="file" multiple className="hidden" onChange={(e) => { void pick(e.target.files); e.target.value = ""; }} />
          <input ref={photoRef} type="file" accept="image/*" multiple className="hidden" onChange={(e) => { void pick(e.target.files); e.target.value = ""; }} />
          <input ref={cameraRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => { void pick(e.target.files); e.target.value = ""; }} />

          <div className="flex items-end gap-1.5 rounded-3xl border border-border bg-surface px-2 py-1.5 shadow-sm">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button title="Add" className="flex size-9 shrink-0 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-surface-2 hover:text-foreground">
            ...
            {voiceSupported && (
              <button
                ...
                className={`flex size-9 shrink-0 items-center justify-center rounded-full transition-colors ${
                  listening ? "bg-destructive/15 text-destructive" : "text-muted-foreground hover:bg-surface-2 hover:text-foreground"
                }`}
              >
