...
class _NexusChatScreenState extends State<NexusChatScreen> {
  ...
  void send() {
    ...
    setState(() { messages.add({'role': 'user', 'text': text}); controller.clear(); busy = true; });
    Future<void>.delayed(const Duration(milliseconds: 300), () {
      if (!mounted) return;
      setState(() { busy = false; messages.add({'role': 'assistant', 'text': 'JagX AI is ready.'}); });
    });
  }
  void modes() {
  ...
  @override Widget build(BuildContext context) {
    return Column(children: [
      Expanded(
        child: messages.isEmpty ? const SizedBox.expand() : ListView.builder(
          padding: const EdgeInsets.all(18),
            ...
            return Padding(
              ...
              child: Align(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(color: JagxColors.elevated, borderRadius: BorderRadius.circular(18)),
                  child: SelectableText(message['text'] ?? '', style: const TextStyle(color: JagxColors.fg, height: 1.5)),
                ),
              ),
            );
          },
      ...
      if (busy) const Padding(padding: EdgeInsets.only(bottom: 6), child: Text('Working…', style: TextStyle(color: JagxColors.muted, fontSize: 12))),
      SafeArea(
        top: false,
        child: Padding(
